# Claude Code Project Instructions — portable-pty-kotlin

## Project Overview

This is **portable-pty-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`portable-pty`](https://crates.io/crates/portable-pty).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/portable-pty/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/portable-pty>
