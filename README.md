...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`portable-pty`](https://crates.io/crates/portable-pty).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `portable-pty` crate will be cloned into `tmp/` (gitignored) when porting begins.

## About

Cross-platform pseudo-terminal allocator
...
Original work copyrighted by the upstream `portable-pty` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
